...
// separate `spark_internal` (no company_id) case covered elsewhere. AppLayout
// also keys the compliance provider on the acting company so a Spark user
// switching tenants doesn't carry the previous one's polled runs across.
vi.mock("@/context/AuthContext", () => ({
  useAuth: () => ({
    user: { user_id: "u1", company_id: "co-1", role: "admin" },
    actingCompany: null,
  }),
}));
